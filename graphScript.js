class Graph {
    ...
    showCorner(cornerColor) {
        var cornerPick = "";
        switch(cornerColor) {
            case "yellow":
                cornerPick = cornerColor
                break;
            case "green":
                cornerPick = cornerColor
                break;
        ...
        document.getElementById(cornerPick.concat("Fill")).setAttribute("fill-opacity","0.5")
        document.getElementById(cornerPick.concat("Outline")).setAttribute("stroke-width","9")

    }
    hideCorners() {
        var corners = ["yellow", "green", "blue", "red"]

        for (const corner in corners) {
            console.log();
            document.getElementById(corners[corner].concat("Fill")).setAttribute("fill-opacity","0")
            document.getElementById(corners[corner].concat("Outline")).setAttribute("stroke-width","0")
        }
    }
    showCatIndex(percentage) {
        var regulatedPercent = Math.max(Math.min(percentage, 100), 0);
        var offset = 5039 - ((5039 * regulatedPercent) / 100);
        document.getElementById("catIndex").setAttribute("stroke-dashoffset", offset.toString());
    }
    dehighlightIssues() {
...
document.getElementById("btnCorner").addEventListener("click", showCornerAUX);
function showCornerAUX() {
    graph.showCorner(document.getElementById("showCorner").value);
}
document.getElementById("btnCornersHide").addEventListener("click", hideCornersAUX);
function hideCornersAUX() {
    graph.hideCorners();
}
document.getElementById("btnCatIndex").addEventListener("click", showCatIndexAUX);
function showCatIndexAUX() {
    graph.showCatIndex(document.getElementById("showCatIndex").value);
}
